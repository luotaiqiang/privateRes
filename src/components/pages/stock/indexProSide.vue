    ...
    <div v-for="pro in proList" :key="pro.id">
      ...
      <div class="pro-item">
        ...
        <div class="pro-detail" :class="{ hide:detail }">
          ...
          <p><span class="bold">公司网站：</span><a class="color-default" target="_blank" :href="pro.proLink">{{pro.proLink}}</a></p>
          <p><span class="bold">公司地址：</span>{{pro.proAddress}}</p>
      ...
      <div class="pro-item">
        ...
        <ul class="pro-item-ul">
          <li v-for="item in pro.eventList" :key="item.id">{{item.event}}</li>
        </ul>
      </div>
      <!-- 行业标签 -->
      <div class="pro-item">
        <div class="pro-title">行业标签：{{pro.proLabel}}</div>
